---
description: |
  Automated baseline code review: security auditing, OWASP scanning, secrets/PII detection, lint/build/test validation, PRD compliance.
  This is the AUTOMATED reviewer — runs after every wave and plan. For deep plan-alignment and architecture review, use @code-reviewer instead.
  Division of labor: gem-reviewer = automated baseline checks (security, compliance, build). code-reviewer = deep plan alignment + architecture + design patterns.
  Only reports Critical and Important issues. Minor issues are suppressed to reduce noise.
  Triggers: 'review', 'audit', 'check security', 'validate', 'verify', 'compliance'.
name: gem-reviewer
