...
1. **Read the current milestone** from `production/milestones/` using `file_search` and `read_file`.

2. **Read the previous sprint** (if any) from `production/sprints/` to understand velocity and carryover.

3. **Scan design documents** in `design/gdd/` using `file_search` for features tagged as ready for implementation.

4. **Check the risk register** at `production/risk-register/` using `read_file`.
...
**Generate a sprint plan** following this format and present it to the user. Do NOT write yet — the producer feasibility gate (Phase 4) runs first and may require revisions before the file is written.
...
- [ ] Smoke check passed (`/cc-smoke-check sprint`)
- [ ] QA sign-off report: APPROVED or APPROVED WITH CONDITIONS (`/cc-team-qa sprint`)
...
After generating a new sprint plan, also write `production/sprint-status.yaml`. This is the machine-readable source of truth for story status — read by `/cc-sprint-status`, `/cc-story-done`, and `/cc-help` without markdown parsing.
...
```yaml
# Auto-generated by /cc-sprint-plan. Updated by /cc-story-done.
# DO NOT edit manually — use /cc-story-done to update story status.
...
Before finalising the sprint plan, spawn `cc-producer` via `runSubagent` using gate **PR-SPRINT**.
...
After handling the producer's verdict, ask: "May I write this sprint plan to `production/sprints/sprint-[N].md`?" If yes, use `create_file` to write the file, creating the directory if needed. Verdict: **COMPLETE** — sprint plan created. If no: Verdict: **BLOCKED** — user declined write.

After writing, add:

> **Scope check:** If this sprint includes stories added beyond the original epic scope, run `/cc-scope-check [epic]` to detect scope creep before implementation begins.
...
Use `file_search` to look for `**/qa-plan-sprint-[N].md` or any file in `production/qa/` referencing this sprint number.
...
>
> Run `/cc-qa-plan sprint` now, before starting any implementation. It takes one session and produces the test case requirements each story needs."

Use `vscode_askQuestions`:
- Prompt: "No QA plan found for this sprint. How do you want to proceed?"
- Options:
  - `[A] Run /cc-qa-plan sprint now — I'll do that before starting implementation (Recommended)`
  - `[B] Skip for now — I understand QA sign-off will be blocked at the Production → Polish gate`

If [A]: close with "Sprint plan written. Run `/cc-qa-plan sprint` next — then begin implementation."
If [B]: add a warning block to the sprint plan document:

```markdown
> ⚠️ **No QA Plan**: This sprint was started without a QA plan. Run `/cc-qa-plan sprint`
...
- `/cc-qa-plan sprint` — **required before implementation begins** — defines test cases per story so developers implement against QA specs, not a blank slate
- `/cc-story-readiness [story-file]` — validate a story is ready before starting it
- `/cc-dev-story [story-file]` — begin implementing the first story
- `/cc-sprint-status` — check progress mid-sprint
- `/cc-scope-check [epic]` — verify no scope creep before implementation begins
