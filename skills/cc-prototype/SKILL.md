...
If yes, use `create_file` to create the directory and files. Every file must begin with:
...
Run the prototype via `run_in_terminal`. Observe behavior. Collect any measurable data (frame times, interaction counts, feel assessments).
...
If yes, use `create_file` to write the file.
...
Spawn `cc-creative-director` via `runSubagent` using gate **CD-PLAYTEST**.
...
If **PROCEED**: run `/cc-design-system` to begin the production GDD for this mechanic, or `/cc-architecture-decision` to record key technical decisions before implementation.
...
- **If PROCEED**: Run `/cc-design-system [mechanic]` to author the production GDD, or `/cc-architecture-decision` to record key technical decisions before implementation
- **If PIVOT**: Run `/cc-prototype [revised-concept]` to test the adjusted direction
- **If KILL**: No further action required — the prototype report is the deliverable
- Run `/cc-playtest-report` to formally document any playtest sessions conducted during prototyping
