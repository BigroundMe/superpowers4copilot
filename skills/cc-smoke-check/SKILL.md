...
Arguments can be combined: `/cc-smoke-check sprint --platform console`
...
1. **Test framework check**: verify `tests/` directory exists using `list_dir`.
   If it does not: "No test directory found at `tests/`. Run `/cc-test-setup`
...
3. **Engine detection**: read `.claude/docs/technical-preferences.md` using `read_file`
   and extract the `Engine:` value. Store this for test command selection in
   Phase 2.
...
5. **QA plan check**: use `file_search` with pattern `production/qa/qa-plan-*.md` and take the most
   ...
   `/cc-qa-plan sprint` before smoke-checking for best results."
...
Attempt to run the test suite via `run_in_terminal`. Select the command based on the engine
detected in Phase 1:
...
`/cc-setup-engine` to specify the engine, then re-run `/cc-smoke-check`."
...
3. If the `quick` argument was passed, skip this phase entirely and note:
   "Coverage scan skipped — run `/cc-smoke-check sprint` for full coverage
   analysis."
...
2. Use `file_search` with patterns `tests/unit/[system]/*` and `tests/integration/[system]/*` for files
...
appear prominently in the report and must be resolved before `/cc-story-done` can
...
Use `vscode_askQuestions` to batch-verify. Keep to at most 3 calls.

**Batch 1 — Core stability (always run):**
- Game launches to main menu without crash — PASS / FAIL
- New game / session starts successfully — PASS / FAIL
- Main menu responds to all inputs — PASS / FAIL

**Batch 2 — Sprint mechanic and regression (always run):**
- [Primary mechanic this sprint] — PASS / FAIL
- [Second notable change this sprint, if any] — PASS / FAIL
- Previous sprint's features still work (no regressions) — PASS / FAIL

**Batch 3 — Data integrity and performance (run unless `quick` argument):**
- Save / load completes without data loss — PASS / FAIL / N/A
- No new frame rate drops or hitches observed — PASS / FAIL / not checked
...
- Keyboard controls work correctly across all menus and gameplay
- Mouse input and cursor visibility correct in all states
- Windowed and fullscreen modes function without graphical issues
- Resolution changes apply correctly

**Console platform** (`--platform console` or `--platform all`):
- Gamepad input works correctly for all actions
- UI fits within TV safe zone margins (no text clipped)
- No keyboard/mouse-only fallbacks shown to gamepad user
- Game boots correctly from cold start (no prior save)

**Mobile platform** (`--platform mobile` or `--platform all`):
- Touch controls work correctly for all primary actions
- Game handles orientation change (portrait ↔ landscape) correctly
- Background / foreground transitions (home button) handled gracefully
- No visible performance issues on target device
...
**Engine**: [engine]
**QA Plan**: [path, or "Not found — run /cc-qa-plan first"]
...
Fix the failures and run `/cc-smoke-check` again to re-gate before QA hand-off."

**If verdict is PASS WITH WARNINGS:**

"Smoke check passed with warnings. The build is ready for manual QA.

Advisory items to resolve before running `/cc-story-done` on affected stories:
[list MISSING test evidence entries]

QA hand-off: share `production/qa/qa-plan-[sprint].md` with the cc-qa-tester
