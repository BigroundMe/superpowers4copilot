...
- **`sprint`** — read the most recent file in `production/sprints/`, extract
  ...
  metadata.
- **`feature: [system-name]`** — use `file_search` with pattern `production/epics/*/story-*.md`, filter
...
For each in-scope story file, use `read_file` to read the full file and extract:
...
**Who must sign off**: [designer / cc-lead-programmer / cc-qa-lead / art-lead]
...
*Smoke tests are verified by the developer via `/cc-smoke-check`. Reference this
...
the [designer / cc-qa-lead] before the story can be marked COMPLETE.
````

Ask: "May I write this QA plan to `production/qa/qa-plan-[sprint-slug]-[date].md`?"
...
- Share the plan with the cc-qa-tester agent for manual test preparation.
- Developers should reference the "Automated Tests Required" section when
  writing tests during `/cc-dev-story`.
- Run `/cc-smoke-check sprint` after implementation to verify against this plan.
- Run `/cc-test-evidence-review sprint` before QA sign-off to verify test quality.
