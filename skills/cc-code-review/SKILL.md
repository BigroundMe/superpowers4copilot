...
Read the target file(s) in full using `read_file`. Read CLAUDE.md for project coding standards.

---

## Phase 2: Identify Engine Specialists

Read technical preferences documentation, section `## Engine Specialists`. Note:
...
Use `grep_search` for ADR references in the story file, commit messages, and header comments. Look for patterns like `ADR-NNN` or `docs/architecture/ADR-`.

If no ADR references found, note: "No ADR references found — skipping ADR compliance check."

For each referenced ADR: read the file using `read_file`, extract the **Decision** and **Consequences** sections, then classify any deviation:
...
Spawn all applicable specialists simultaneously via `runSubagent` — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `cc-qa-tester` via `runSubagent` in parallel with the engine specialists. Pass:
- The implementation files being reviewed
- The story's `## QA Test Cases` section (the pre-written test specs from cc-qa-lead)
- The story's `## Acceptance Criteria`

Ask the cc-qa-tester to evaluate:
- [ ] Are all test hooks and interfaces exposed?
- [ ] Do the QA test cases from the story map to testable code paths?
- [ ] Are any acceptance criteria untestable as implemented?
- [ ] Does the implementation introduce any new edge cases not covered by existing QA test cases?
- [ ] Are there any observable side effects that should have a test but don't?

For Visual/Feel and UI stories: cc-qa-tester reviews whether the manual verification steps in `## QA Test Cases` are achievable with the implementation as written.
...
[cc-qa-tester findings: test hooks, coverage gaps, untestable paths, new edge cases]
...
- If verdict is APPROVED: run `/cc-story-done [story-path]` to close the story.
- If verdict is CHANGES REQUIRED: fix the issues and re-run `/cc-code-review`.
- If an ARCHITECTURAL VIOLATION is found: run `/cc-architecture-decision` to record the correct approach.
