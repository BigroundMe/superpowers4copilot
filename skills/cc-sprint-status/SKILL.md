...
This is a fast situational awareness check, not a sprint review. It reads the current sprint plan and story files, scans for status markers, and produces a concise snapshot in under 30 lines. For detailed sprint management, use `/cc-sprint-plan update` or `/cc-milestone-review`.
...
**Argument:** first argument (blank = use current sprint)

- If an argument is given (e.g., `/cc-sprint-status 3`), use `file_search` to search `production/sprints/` for a file matching `sprint-03.md`, `sprint-3.md`, or similar. Report which file was found.
- If no argument is given, find the most recently modified file in `production/sprints/` and treat it as the current sprint.
- If `production/sprints/` does not exist or is empty, report: "No sprint files found. Start a sprint with `/cc-sprint-plan new`." Then stop.

Read the sprint file in full using `read_file`. Extract:
...
If it exists, read it directly using `read_file` — it is the authoritative source of truth. Extract status for each story from the `status` field. No markdown scanning needed. Use its `sprint`, `goal`, `start`, `end` fields instead of re-parsing the sprint plan.
...
"⚠ No `sprint-status.yaml` found — status inferred from markdown. Run `/cc-sprint-plan update` to generate one."

Optionally (fast check only — do not do a deep scan): use `grep_search` on `src/` for a directory or file name that matches the story's system slug to check for implementation evidence. This is a hint only, not a definitive status.
...
time remaining. Recommend replanning with `/cc-sprint-plan update`.
```
...
Run `/cc-story-readiness sprint` to validate story file coverage.
```
