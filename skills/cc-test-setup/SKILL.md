...
1. **Read engine config**:
   - Use `read_file` on `.claude/docs/technical-preferences.md` and extract the `Engine:` value.
   - If engine is not configured (`[TO BE CONFIGURED]`), stop:
     "Engine not configured. Run `/cc-setup-engine` first, then re-run `/cc-test-setup`."

2. **Check for existing test infrastructure**:
   - Use `file_search` for `tests/` — does the directory exist?
   - Check `tests/unit/` and `tests/integration/` — do subdirectories exist?
   - Check `.github/workflows/` — does a CI workflow file exist?
   - Check `tests/gdunit4_runner.gd` (Godot) or `tests/EditMode/` (Unity) or
...
3. **Report findings**:
   ...
   - If everything already exists AND `force` argument was not passed:
     "Test infrastructure appears to be in place. Re-run with `/cc-test-setup force`
...
After approval, use `create_file` to create the following files:
...
tests/
  ...
  smoke/          # Critical path test list for /cc-smoke-check gate
...
```gdscript
# GdUnit4 test runner — invoked by CI and /cc-smoke-check
...
Create `tests/EditMode/README.md`:
```markdown
# Edit Mode Tests
...
Create `tests/smoke/critical-paths.md` using `create_file`:

```markdown
# Smoke Test: Critical Paths

**Purpose**: Run these 10-15 checks in under 15 minutes before any QA hand-off.
**Run via**: `/cc-smoke-check` (which reads this file)
...
3. Run `/cc-qa-plan sprint` before your first sprint to classify stories and set
   test expectations.
4. Run `/cc-test-helpers scaffold` to generate base helper libraries.
```
