...
Read CLAUDE.md for project overview and standards using `read_file`.

Read the relevant agent definition from the agents directory if a specific role is specified, using `file_search` and `read_file`.

---

## Phase 2: Scan Relevant Area

- For programmers: scan `src/` using `list_dir` and `read_file` for architecture, patterns, key files
...
Read recent changes using `run_in_terminal` (`git log --oneline -20`) to understand current momentum.
...
- Run `/cc-sprint-status` to show the new contributor current progress.
- Run `/cc-help` if the contributor needs guidance on what to work on next.
