...
Read relevant design documents from `design/gdd/` using `file_search` and `read_file` if the task relates to a documented feature or system.
...
- Identify integration points with other systems using `grep_search`
- Check for existing test coverage in the affected areas using `file_search`
...
- If confidence is Low: recommend a time-boxed spike (`/cc-prototype`) before committing.
- If the task is > 10 days: recommend breaking it into smaller stories via `/cc-create-stories`.
- To schedule the task: run `/cc-sprint-plan update` to add it to the next sprint.
