...
2. **Search the codebase** for related files using `grep_search` / `file_search` to add context (affected system, likely files).
...
1. **Read the target file(s)** specified in the argument using `read_file`.
...
Read `production/qa/bugs/[BUG-ID].md` using `read_file`. Extract the reproduction steps and expected result.

1. **Re-run reproduction steps** — use `grep_search` / `file_search` to check whether the root cause code path still exists as described. If the fix removed or changed it, note the change.
2. **Run the related test** — if the bug's system has a test file in `tests/`, run it via `run_in_terminal` and report pass/fail.
3. **Check for regression** — use `grep_search` on the codebase for any new occurrence of the pattern that caused the bug.
...
If STILL PRESENT: reopen the bug, set Status back to Open, and suggest re-running `/cc-hotfix [BUG-ID]`.

---

## Phase 2D: Close Mode

Read `production/qa/bugs/[BUG-ID].md` using `read_file`. Confirm Status is `Verified Fixed` before closing. If status is anything else, stop: "Bug [ID] must be Verified Fixed before it can be closed. Run `/cc-bug-report verify [BUG-ID]` first."

Append a closure record to the bug file using `replace_string_in_file`:
...
After closing, check `production/qa/bug-triage-*.md` — if the bug appears in an open triage report, note: "Bug [ID] is referenced in the triage report. Run `/cc-bug-triage` to refresh the open bug count."
...
If yes, use `create_file` to write the file, creating the directory if needed. Verdict: **COMPLETE** — bug report filed.
...
- Run `/cc-bug-triage` to prioritize alongside existing open bugs
- If S1 or S2: run `/cc-hotfix [BUG-ID]` for emergency fix workflow

**After fixing the bug (developer confirms fix is in):**
- Run `/cc-bug-report verify [BUG-ID]` — confirm the fix actually works before closing
- Never mark a bug closed without verification — a fix that doesn't verify is still Open

**After verify returns VERIFIED FIXED:**
- Run `/cc-bug-report close [BUG-ID]` — write the closure record and update status
- Run `/cc-bug-triage` to refresh the open bug count and remove it from the active list
