...
- After `/cc-team-qa` completes and new bugs have been filed
...
**Modes:**
- `/cc-bug-triage sprint` — triage against the current sprint; assign fixable bugs to the sprint backlog; defer the rest
- `/cc-bug-triage full` — full triage of all bugs regardless of sprint scope
- `/cc-bug-triage trend` — trend analysis only (no assignment); read-only report
...
Use `file_search` for bug reports in priority order:
...
Read the most recently modified file in `production/sprints/` using `read_file` to understand:
...
Read coding standards documentation for severity/priority definitions if they exist. If they do not exist, use the standard definitions in Step 3.
...
Use `create_file` to write only after approval.

After writing:
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint can be considered healthy. Run `/cc-sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found — consider re-opening the affected stories in sprint tracking and running `/cc-smoke-check` to re-gate."
