...
- Before a release gate (`/cc-gate-check polish` requires regression suite exists)
...
**Modes:**
- `/cc-regression-suite update` — scan new bug fixes this sprint and check
  for regression test presence; add new tests to the suite manifest
- `/cc-regression-suite audit` — full audit of all GDD critical paths vs.
  existing test coverage; flag paths with no regression test
- `/cc-regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
...
Use `read_file` on `tests/regression-suite.md` if it exists. Extract:
...
Use `file_search` to find all test files:
```
tests/unit/**/*_test.*
...
For `audit` mode: use `read_file` on `design/gdd/systems-index.md` to get all systems.
...
Use `file_search` with pattern `production/qa/bugs/*.md` and filter for bugs with a `Status: Closed`
...
1. Use `grep_search` in `tests/unit/[system]/` and `tests/integration/[system]/` for file names
...
2. Use `grep_search` in `tests/unit/[system]/` and `tests/integration/[system]/` for a test
...
For `update` mode: append new entries using `replace_string_in_file`; never remove existing entries.
For `audit` mode: rewrite the full manifest using `create_file` with updated coverage data.
...
- For each HIGH priority gap: "Consider creating the missing regression test
  before the next sprint. Run `/cc-test-helpers` to scaffold the test file."
...
- If coverage drift detected: "Regression suite may be drifting. Consider
  running `/cc-regression-suite audit` at the next sprint boundary."
