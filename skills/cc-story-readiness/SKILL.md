...
**Scope:** first argument (blank = ask user via `vscode_askQuestions`)

- **Specific path** (e.g., `/cc-story-readiness production/epics/combat/story-001-basic-attack.md`): validate that single story file.
...
- **`all`**: use `file_search` for `**/production/epics/**/*.md`, exclude `EPIC.md` index files, validate every story file found.
- **No argument**: ask the user which scope to validate.

If no argument is given, use `vscode_askQuestions`:
...
- `docs/architecture/tr-registry.yaml` — index all entries by `id`. Used to validate TR-IDs in stories. If the file does not exist, note it once; TR-ID checks will auto-pass for all stories.
...
- [ ] **No acceptance criteria require judgment calls**: Criteria like "feels responsive" or "looks good" are not testable without a defined benchmark. These must be replaced with specific observable conditions or playtest protocols.

### Architecture Completeness

- [ ] **ADR referenced or N/A stated**: The story references at least one ADR, OR explicitly states "No ADR applies" with a brief reason.
...
- [ ] **TR-ID is valid and active**: If the story contains a `TR-[system]-NNN` reference, look it up in the TR registry loaded in Section 2.
  ...
  - If the ID exists and `status: deprecated` or `status: superseded-by: ...` → NEEDS WORK.
  - If the ID does not exist in the registry → NEEDS WORK.
...
- [ ] **Manifest version is current**: If the story has a `Manifest Version:` date in its header AND `docs/architecture/control-manifest.md` exists:
  - If story version matches current manifest → pass.
  - If story version is older → NEEDS WORK.
  - Auto-pass if either the story has no `Manifest Version:` field OR the manifest does not exist.
- [ ] **Engine notes present**: For any post-cutoff engine API this story is likely to touch, implementation notes or a verification requirement are included. If the story clearly does not touch engine APIs, "N/A — no engine API involved" is acceptable.
- [ ] **Control manifest rules noted**: Relevant layer rules from the control manifest are referenced, OR "N/A — manifest not yet created" is stated. Auto-passes if `docs/architecture/control-manifest.md` does not exist yet.

### Scope Clarity

- [ ] **Estimate present**: The story includes a size estimate (hours, points, or a t-shirt size).
- [ ] **In-scope / Out-of-scope boundary stated**: The story states what it does NOT include.
- [ ] **Story dependencies listed**: If this story depends on other stories being DONE first, those story IDs are listed. If there are no dependencies, "None" is explicitly stated.
...
- [ ] **Dependency stories are not in DRAFT**: For each story listed as a dependency, check if the file exists and does not have a DRAFT status. A story that depends on a DRAFT or missing story is BLOCKED.
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns (paths containing `assets/`, or file extensions `.png`, `.jpg`, `.svg`, `.wav`, `.ogg`, `.mp3`, `.glb`, `.gltf`, `.tres`, `.tscn`, `.res`).
  - For each asset path found: use `file_search` to check whether the file exists.
  - If any referenced asset does not exist: **NEEDS WORK**.
  - If all exist: note "Referenced assets verified: [count] found."
  - If no asset paths are referenced: note "No asset references found in story — skipping asset check." Auto-passes.

### Definition of Done

- [ ] **At least 3 testable acceptance criteria**: Fewer than 3 suggests the story is either trivially small or under-specified.
...
- [ ] **Story Type declared**: The story includes a `Type:` field in its header identifying the test category (Logic / Integration / Visual/Feel / UI / Config/Data).
- [ ] **Test evidence requirement is clear**: If the Story Type is set, the story includes a `## Test Evidence` section stating where evidence will be stored.
...
**BLOCKED** — One or more dependency stories are missing or in DRAFT state, OR a critical design question has no owner. The story cannot be assigned until the blocker is resolved.
...
If the scope is `sprint` and any Must Have stories are NEEDS WORK or BLOCKED, add a prominent warning at the top:

```
WARNING: [N] Must Have stories are not implementation-ready.
[List them with their primary gap or blocker.]
Resolve these before the sprint begins or replan with `/cc-sprint-plan update`.
```
...
If the user says yes for a specific story, draft only the missing sections in conversation. Do not use `create_file` or `replace_string_in_file` — the user (or `/cc-create-stories`) handles writing.

**Redirect rules:**
- If a story file does not exist at all: "This story file is missing entirely. Run `/cc-create-epics [layer]` then `/cc-create-stories [epic-slug]` to generate stories from the GDD and ADR."
- If a story has no GDD reference and the work appears small: "This story has no GDD reference. If the change is small (under ~4 hours), run `/cc-quick-design [description]` to create a Quick Design Spec."
...
2. Find stories that are: Status READY or NOT STARTED, not the story just checked, not blocked, in Must Have or Should Have tier.
...
Run `/cc-story-readiness [path]` to validate before starting.
```
...
Spawn `cc-qa-lead` via `runSubagent` using gate **QL-STORY-READY**.

Pass: story title, acceptance criteria list, dependency status, overall verdict.

Handle the verdict:
- **ADEQUATE** → story is cleared. Proceed to close.
- **GAPS** → surface the specific gaps to the user via `vscode_askQuestions`: options: `Update story with suggested gaps` / `Accept and proceed anyway` / `Discuss further`.
...
- Run `/cc-dev-story [story-path]` to begin implementation once the story is READY
- Run `/cc-story-readiness sprint` to check all stories in the current sprint at once
- Run `/cc-create-stories [epic-slug]` if a story file is missing entirely
