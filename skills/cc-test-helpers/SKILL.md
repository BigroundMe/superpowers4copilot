...
**When to run:**
- After `/cc-test-setup` scaffolds the framework (first time)
...
**Modes:**
- `/cc-test-helpers [system-name]` — generate helpers for a specific system
  (e.g., `/cc-test-helpers combat`)
- `/cc-test-helpers all` — generate helpers for all systems with test files
- `/cc-test-helpers scaffold` — generate only the base helper library (no
...
Use `read_file` on `.claude/docs/technical-preferences.md` and extract:
- `Engine:` value
- `Language:` value
- `Framework:` from the Testing section

If engine is not configured: "Engine not configured. Run `/cc-setup-engine` first."
...
Use `file_search` with pattern `tests/**/*_test.*` to find all test files.

For a representative sample (up to 5 files), use `read_file` on the test files and extract:
...
Read the system's GDD using `read_file` to extract:
...
## Generated by /cc-test-helpers combat on [date].
...
After writing using `create_file`: Verdict: **COMPLETE** — helper files created.
...
- Run `/cc-test-setup` if the test framework has not been scaffolded yet.
- Use `/cc-dev-story` to implement stories — helpers reduce boilerplate in new test files.
- Run `/cc-test-evidence-review` to validate test quality after writing tests.
