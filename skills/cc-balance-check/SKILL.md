...
Determine the balance domain from the argument:
...
If no argument, ask the user which system to check using `vscode_askQuestions`.

---

## Phase 2: Read Data Files

Use `read_file` on relevant files from `assets/data/` and `design/balance/` for the identified domain.
Use `file_search` and `grep_search` to locate the data files.
...
Use `read_file` on the GDD for the system from `design/gdd/` to understand intended design targets,
...
- If the fix changes a tuning knob defined in a GDD or referenced by an ADR, remind the user:
  > "This value is defined in a design document. Run `/cc-propagate-design-change [path]` on the affected GDD to find downstream impacts before committing."

If no:
- Summarize open issues and suggest saving the report to `design/balance/balance-check-[system]-[date].md` for later

End with:
> "Re-run `/cc-balance-check` after fixes to verify."
