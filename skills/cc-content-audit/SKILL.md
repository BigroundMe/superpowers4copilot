...
1. **Read `design/gdd/systems-index.md`** using `read_file` for the full list of systems, their
   categories, and MVP/priority tier.

2. **L0 pre-scan**: Before full-reading any GDDs, use `grep_search` on all GDD files for
   `## Summary` sections plus common content-count keywords:
   ```
   Pattern: (## Summary|N enemies|N levels|N items|N abilities|enemy types|item types)
   Include: design/gdd/*.md
   ```
...
3. **Full-read in-scope GDD files** using `read_file` (or the single system GDD if a system
   name was given).
...
For each content type found in Phase 1, scan the relevant directories using
`file_search` and `grep_search` to count what has been implemented.

**Levels / Areas / Maps:**
- Use `file_search` with patterns `assets/**/*.tscn`, `assets/**/*.unity`, `assets/**/*.umap`
...
- Use `file_search` with patterns `assets/data/**/enemies/**`, `assets/data/**/characters/**`
- Check `src/**/enemies/**`, `src/**/characters/**`
- Look for `.json`, `.tres`, `.asset`, `.yaml` data files defining entity stats

**Items / Equipment / Loot:**
- Use `file_search` with patterns `assets/data/**/items/**`, `assets/data/**/equipment/**`,
  `assets/data/**/loot/**`
- Look for `.json`, `.tres`, `.asset` data files

**Abilities / Skills / Spells:**
- Use `file_search` with patterns `assets/data/**/abilities/**`, `assets/data/**/skills/**`,
  `assets/data/**/spells/**`
- Look for `.json`, `.tres`, `.asset` data files

**Dialogue / Conversations / Cutscenes:**
- Use `file_search` with patterns `assets/**/*.dialogue`, `assets/**/*.csv`, `assets/**/*.ink`
- Use `grep_search` for dialogue data files in `assets/data/`

**Quests / Missions:**
- Use `file_search` with patterns `assets/data/**/quests/**`, `assets/data/**/missions/**`
...
If yes, write the file using `create_file`:
...
to `/cc-create-stories [epic-slug]` or `/cc-quick-design` depending on the size of the gap.

### --summary mode

Print the Gap Table and Summary directly to conversation. Do not write a file.
End with: "Run `/cc-content-audit` without `--summary` to write the full report."
...
- If any system is `NOT STARTED` and MVP-tagged → "Run `/cc-design-system [name]` to
  add missing content counts to the GDD before implementation begins."
- If total gap is >50% → "Run `/cc-sprint-plan` to allocate content work across upcoming sprints."
- Run `/cc-asset-audit` to cross-check asset compliance with naming and format standards.
