...
**Argument:** first argument — feature name, sprint number, or milestone name.

---

## Phase 1: Find the Original Plan

Locate the baseline scope document for the given argument using `file_search` and `read_file`:
...
- Scan the codebase for files related to the feature/sprint using `file_search` and `grep_search`
- Read git log for commits related to this work using `run_in_terminal` (`git log --oneline --since=[start-date]`)
- Check for TODO/FIXME comments that indicate unfinished scope additions using `grep_search`
...
- **CONCERNS** → offer to identify the 2–3 additions with best cut ratio. Reference `/cc-sprint-plan update` to formally re-scope.
- **FAIL** → recommend escalating to producer. Reference `/cc-sprint-plan update` for re-planning or `/cc-estimate` to re-baseline timeline.

Always end with:
> "Run `/cc-scope-check [name]` again after cuts are made to verify the verdict improves."
