...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `/cc-hotfix`. Do not auto-invoke based on context matching.
...
### Approvals
- [ ] Fix reviewed by cc-lead-programmer
- [ ] Regression test passed (cc-qa-tester)
- [ ] Release approved (cc-producer)
...
If yes, use `create_file` to write the file, creating the directory if needed.

---

## Phase 3: Create Hotfix Branch

If git is initialized, create the hotfix branch via `run_in_terminal`:
...
Update the hotfix record with root cause, fix details, and test results using `replace_string_in_file`.

---

## Phase 5: Collect Approvals

Use `runSubagent` to request sign-off in parallel:

- `cc-lead-programmer` — Review the fix for correctness and side effects
- `cc-qa-tester` — Run targeted regression tests on the affected system
- `cc-producer` — Approve deployment timing and communication plan
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `cc-qa-lead` via `runSubagent` with:
...
Ask cc-qa-lead: **Is a full smoke check sufficient, or does this fix require a targeted team-qa pass?**

Apply the verdict:
- **Smoke check sufficient** — run `/cc-smoke-check` against the hotfix build. If PASS, proceed to Phase 6.
- **Targeted QA pass required** — run `/cc-team-qa [affected-system]` scoped to the changed system only. If QA returns APPROVED or APPROVED WITH CONDITIONS, proceed to Phase 6.
- **Full QA required** — S1 fixes that touch core systems may require a full `/cc-team-qa sprint`. This delays deployment but prevents a bad patch.
...
Update the original bug file if one exists using `replace_string_in_file`:
...
**Approvals**: cc-lead-programmer ✓ / cc-qa-tester ✓ / cc-producer ✓
**Rollback plan**: [from Phase 2 record]

Merge to: release branch AND development branch
Next: /cc-bug-report verify [BUG-ID] after deploy to confirm resolution
```
...
- If the fix is complex enough to need more than 4 hours, escalate to `cc-technical-director`

---

## Phase 7: Post-Deploy Verification

After deploying, run `/cc-bug-report verify [BUG-ID]` to confirm the fix resolved the issue in the deployed build.

If VERIFIED FIXED: run `/cc-bug-report close [BUG-ID]` to formally close it.
If STILL PRESENT: the hotfix failed — immediately re-open, assess rollback, and escalate.

Schedule a post-incident review within 48 hours using `/cc-retrospective hotfix`.
